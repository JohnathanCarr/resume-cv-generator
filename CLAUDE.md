...
Output requirements agreed for the rewrite: a first **match-analysis** call turns the JD into `{company, role, seniority, requirements[{text, kind, evidence[profile ids], strength}], keywords[]}` that both generators consume; resumes mirror JD keyword phrasing only where the profile has the skill, use standard section headings, single column, verb+what+result bullets, and never invent numbers; cover letters are 250–350 words in 3–4 paragraphs, voiced by two exemplar paragraphs instead of a phrase ban-list, and every claim must trace to a profile item (`claims[{sentence, source}]`, unsourced → reject). Company research runs as a separate call using OpenAI's hosted web-search tool and produces a **company brief** (what they build, what they care about, current priorities, challenges, where the candidate fits) with a source per item; thin footprint → lean on the JD, never fill gaps with invention.
